...
//        blocklyDiv.style.top = metrics.absoluteTop + 'px';
    ...
    window.addEventListener('pagehide', auto_save_and_restore_blocks, false);
    ...
    function dragElement(element, direction, first, second) {
        ...
        function onMove(e) {
            ...
            if (direction === "H") {
                delta.x = Math.min(Math.max(delta.x, -dragInfo.firstWidth), dragInfo.secondWidth);
                element.style.left = dragInfo.offsetLeft + delta.x + "px";
                first.style.width = (dragInfo.firstWidth + delta.x) + "px";
                second.style.width = (dragInfo.secondWidth - delta.x) + "px";

                const contentCode = document.getElementById("content_code");
                const copyButton = document.getElementById("copyCodeButton");
                if (contentCode && copyButton) {
                    copyButton.style.visibility = contentCode.offsetWidth < 50 ? "hidden" : "visible";
                }
                console.log(contentCode.offsetWidth);
            }

            if (direction === "V") {
                delta.y = Math.min(Math.max(delta.y, -dragInfo.firstHeight), dragInfo.secondHeight);
                element.style.top = dragInfo.offsetTop + delta.y + "px";
                first.style.height = (dragInfo.firstHeight + delta.y) + "px";
                second.style.height = (dragInfo.secondHeight - delta.y) + "px";
            }

            onresize();
            if (typeof Blockly !== "undefined" && Blockly.svgResize && Code?.workspace) {
                Blockly.svgResize(Code.workspace);
            }
        }
...
Code.initLanguage = function () {
    ...
    // document.getElementById('registerToOrcshestrator_span_menu').textContent = MSG['registerToOrchestrator_span'];
